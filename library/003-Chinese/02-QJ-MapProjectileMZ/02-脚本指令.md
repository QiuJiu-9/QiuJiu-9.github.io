...
```
修改玩家碰撞体：
$gamePlayer.changeCBQJ(cb,ox,oy);
修改事件碰撞体：
$gameMap.event(eid).changeCBQJ(cb,ox,oy);
在事件页中设置当前事件的碰撞体：
this.character(0).changeCBQJ(cb,ox,oy);

cb:碰撞体，详见【 '04-QJ.MPMZ.Shoot' '17-collisionBox'.】
ox/oy:横向偏移和纵向偏移，写像素值
eid:事件id

范例：
修改玩家碰撞体为宽96高56的矩形,横向偏移0，纵向偏移-8：
$gamePlayer.changeCBQJ(['R',96,48],0,-8);
修改id为7的事件的碰撞体为半径为96的圆,横向偏移0，纵向偏移-48：
$gameMap.event(7).changeCBQJ(['C',96],0,-48);
在事件页中设置当前事件的碰撞体为半径为96的圆,横向偏移0，纵向偏移-48：
this.character(0).changeCBQJ(['C',96],0,-48);
```
